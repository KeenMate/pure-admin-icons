    ...
    async loadSvgs() {
      ...
      for (let i = 0; i < containers.length && i < urls.length; i++) {
        try {
          ...
          const coloredSvg = svgText.replace(/fill="#[0-9A-Fa-f]{3,6}"/g, `fill="${color}"`).replace(/fill="currentColor"/g, `fill="${color}"`).replace(/stroke="#[0-9A-Fa-f]{3,6}"/g, `stroke="${color}"`).replace(/stroke="currentColor"/g, `stroke="${color}"`);
