...
`
  ...
  var THEME_MAP = { morning: "park-morning", day: "park-day", evening: "park-evening", night: "park-night" };
  ...
  function startAutoUpdate() {
    updateTheme();
    setInterval(updateTheme, 6e4);
  }
  var THEME_NAMES = { auto: "Auto", morning: "Morning", day: "Day", evening: "Evening", night: "Night" };
  var THEME_ICONS = {
  ...
  function initThemeSwitcherDropdown() {
  ...
  function initLanguageSwitcherDropdown() {
  ...
  var DesignerExport = {
    ...
    colorizeSvg(svgText, color) {
      ...
      svg.querySelectorAll("path, circle, rect, line, polyline, polygon, ellipse, g").forEach((el) => {
      ...
      const out = new XMLSerializer().serializeToString(doc2);
  ...
  Hooks2.IconColorFilter = {
    ...
    colorizeSvg(svg, color, debugUrl) {
      const hasAny = svg.getAttribute("fill") || svg.getAttribute("stroke") || svg.querySelector("[fill], [stroke]");
  ...
  Hooks2.ColorPicker = {
    mounted() {
      ...
      if (importSubmit) {
        importSubmit.addEventListener("click", () => {
          ...
          const nameInput2 = this.el.querySelector(".custom-preset-name");
    ...
    applySaved() {
      ...
      let savedBg = "#ffffff";
      try {
        savedBg = JSON.parse(localStorage.getItem("icon_preview_bg") || '"#ffffff"');
      } catch {
        savedBg = localStorage.getItem("icon_preview_bg") || "#ffffff";
      }
    ...
    updateSvgColors(color) {
      ...
      if (svgContainer) {
        ...
        svgContainer.querySelectorAll(".svg-container > svg").forEach((svg) => colorizeLiveSvg(svg, color));
      }
    }
  };
  Hooks2.InlineSvg = {
  ...
  Hooks2.SvelteColor = {
    ...
    updateCode(name, style, sizes, includeColor) {
      ...
      codeElements.forEach((code) => {
        ...
        code.textContent = includeColor ? `<Icon name="${name}" size={${size}} variant="${style}" color="custom" customColor="${color}" />` : `<Icon name="${name}" size={${size}} variant="${style}" />`;
      });
    }
  };
  Hooks2.FloatingPopover = {
    mounted() {
      this.attach();
  ...
  Hooks2.DownloadNaming = {
    ...
    applyNaming(convention) {
      ...
      this.el.querySelectorAll(".download-link").forEach((link) => {
        ...
        switch (convention) {
          case "kebab":
            baseName = toKebab(this.name);
            break;
          case "snake":
            baseName = toSnake(this.name);
            break;
          case "pascal":
            baseName = toPascal(this.name);
            break;
          default:
  ...
  Hooks2.FilenameTemplate = {
    ...
    renderFilenames() {
      ...
      resultsContainer.innerHTML = sizes.map((size) => {
        ...
        let filename = template.replace(/{filename}/g, origFilename).replace(/{name}/g, name).replace(/{name_snake}/g, toSnake(name)).replace(/{name_pascal}/g, toPascal(name)).replace(/{name_kebab}/g, toKebab(name)).replace(/{size}/g, size).replace(/{style}/g, style);
  ...
  window.addEventListener("phx:copy_text", (event) => {
    ...
    if (platform === "filename" && filename) {
      ...
      const toPascal = (s) => s.split(/\s+/).map((w) => w.charAt(0).toUpperCase() + w.slice(1).toLowerCase()).join("");
      const toKebab = (s) => s.toLowerCase().replace(/\s+/g, "-");
      text = template.replace(/{filename}/g, filename).replace(/{name}/g, name).replace(/{name_snake}/g, toSnake(name)).replace(/{name_pascal}/g, toPascal(name)).replace(/{name_kebab}/g, toKebab(name)).replace(/{size}/g, size).replace(/{style}/g, style);
    }
    if (text) {
  ...
  window.copyFromButton = function(button) {
    ...
    if (text) {
      navigator.clipboard.writeText(text).then(() => {
        ...
        if (svg) {
          svg.style.color = "#22c55e";
          setTimeout(() => svg.style.color = "", 1e3);
        }
      }).catch((err) => console.error("Failed to copy:", err));
    }
  };
