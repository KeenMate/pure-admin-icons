defmodule PureAdminIconsWeb.IconModalComponent do
          ...
          <div class="p-6">
            ...
            <div class="space-y-4">
              ...
              <div class="flex flex-wrap gap-4 pb-4 border-b border-base-300" id="platform-prefs" phx-hook="PlatformPrefs">
                ...
                <%= if ios_pkg do %>
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={@platform_prefs.ios} phx-click="toggle_platform" phx-value-platform="ios" class="w-4 h-4 rounded border-base-300 focus:ring-primary" />
                ...
                <%= if android_pkg do %>
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={@platform_prefs.android} phx-click="toggle_platform" phx-value-platform="android" class="w-4 h-4 rounded border-base-300 focus:ring-primary" />
                ...
                <%= if react_pkg do %>
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={@platform_prefs.react} phx-click="toggle_platform" phx-value-platform="react" class="w-4 h-4 rounded border-base-300 focus:ring-primary" />
                ...
                <%= if vue_pkg do %>
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={@platform_prefs.vue} phx-click="toggle_platform" phx-value-platform="vue" class="w-4 h-4 rounded border-base-300 focus:ring-primary" />
                ...
                <%= if svelte_pkg do %>
                  <label class="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" checked={@platform_prefs.svelte} phx-click="toggle_platform" phx-value-platform="svelte" class="w-4 h-4 rounded border-base-300 focus:ring-primary" />
              ...
              <%= if @platform_prefs.vue do %>
                ...
                <%= if vue_pkg_name do %>
                  <.identifier_section icon={@icon} platform="vue" pkg_name={vue_pkg_name} pkg_url={vue_pkg_url} color="text-emerald-600" sizes={Formatter.vue_identifier_sizes(@icon)} identifier_fn={&Formatter.vue_identifier(@icon, &1)} />
                <% end %>
              <% end %>
              ...
              <%= if @platform_prefs.cssclass do %>
                ...
                <%= if cssclass_pkg_name do %>
                  <.identifier_section icon={@icon} platform="cssclass" pkg_name={cssclass_pkg_name} pkg_url={cssclass_pkg_url} color="text-pink-600" sizes={Formatter.cssclass_identifier_sizes(@icon)} identifier_fn={&Formatter.cssclass_identifier(@icon, &1)} />
                <% end %>
              <% end %>
              ...
              <%= if @platform_prefs.htmltag do %>
                ...
                <%= if htmltag_pkg_name do %>
                  <.identifier_section icon={@icon} platform="htmltag" pkg_name={htmltag_pkg_name} pkg_url={htmltag_pkg_url} color="text-fuchsia-600" sizes={Formatter.htmltag_identifier_sizes(@icon)} identifier_fn={&Formatter.htmltag_identifier(@icon, &1)} />
                <% end %>
              <% end %>
    ...
    <div class="bg-base-100 rounded-lg p-4">
      <div class="flex items-center justify-between mb-2">
        <span class="text-sm font-medium text-base-content/70 flex items-center gap-1.5">
          ...
          <%= String.capitalize(@platform) %> (<%= if @pkg_url do %><a href={@pkg_url} target="_blank" rel="noreferrer" class="text-primary hover:underline"><%= @pkg_name %></a><% else %><%= @pkg_name %><% end %>)
        </span>
      </div>
      <div class="space-y-1">
  ...
  defp identifier_section(assigns) do
    ...
    <div class="bg-base-100 rounded-lg p-4">
      <div class="flex items-center justify-between mb-2">
        <span class="text-sm font-medium text-base-content/70 flex items-center gap-1.5">
          <.platform_icon name={@platform} class="w-4 h-4" />
          <%= platform_label(@platform) %> (<%= if @pkg_url do %><a href={@pkg_url} target="_blank" rel="noreferrer" class="text-primary hover:underline"><%= @pkg_name %></a><% else %><%= @pkg_name %><% end %>)
        </span>
      </div>
      <div class="space-y-1">
        <%= for size <- @sizes do %>
  ...
  defp preset_button_style(%{"bg" => "checker", "color" => color}) do
    "background-image: repeating-conic-gradient(#d1d5db 0% 25%, #fff 0% 50%); background-size: 6px 6px; color: #{color}; border-color: transparent;"
  end
  defp preset_button_style(%{"bg" => bg, "color" => color}) do
    "background-color: #{bg}; color: #{color}; border-color: transparent;"
  end
